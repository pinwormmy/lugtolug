...
for (let index = 0; index < seed.length; index += WATCH_INSERT_BATCH_SIZE) {
  const batch = seed.slice(index, index + WATCH_INSERT_BATCH_SIZE);
...
const sql = [
  ...
  `UPDATE watches SET reference_slug = 'seed-tmp-' || id WHERE id IN (${seed.map((watch) => watch.id).join(", ")});`,
  "",
